...
CREATE OR REPLACE PROCEDURE "SP_DATA_PROFILER2"("RUN_ID" VARCHAR, "DATASET_ID" VARCHAR, "DBNAME" VARCHAR, "SCHEMANAME" VARCHAR, "TABLENAME" VARCHAR, "CUSTOMSQL" VARCHAR)
RETURNS VARCHAR
LANGUAGE PYTHON
...
def _identify_kanji_numeral_columns(session, snowpark_df, sample_size: int = 1000) -> list:
    ...
    # Note: We keep sampling here intentionally as checking regex on full dataset 
    # just for identification is inefficient.
