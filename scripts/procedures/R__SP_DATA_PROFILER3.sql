...
CREATE OR REPLACE PROCEDURE "SP_DATA_PROFILER3"("RUN_ID" VARCHAR, "DATASET_ID" VARCHAR, "DBNAME" VARCHAR, "SCHEMANAME" VARCHAR, "TABLENAME" VARCHAR, "CUSTOMSQL" VARCHAR)
RETURNS VARCHAR
LANGUAGE PYTHON
...
def run_profiler_sp(session, run_id: str, dataset_id: str, dbname: str, schemaname: str, tablename: str, customsql: str) -> str:
    ...
    # Get the database and schema where the SP is executing (the framework location)
    ...
    # Restructure the report into column and table summary dataframes
